#include <string.h>
#include <jni.h>
#include "../../ios/Classes/beldex_api.cpp"
#include <stdbool.h>
...
struct BeldexWalletListenerWrapper: Beldex::WalletListener {
    jobject listener;

    BeldexWalletListenerWrapper(JNIEnv *env, jobject aListener) {
...
static BeldexWalletListenerWrapper *listenerWrapper = NULL;
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *_jvm, void *reserved) {
    ...
    listener_class = static_cast<jclass>(jenv->NewGlobalRef(jenv->FindClass("io/beldex/coin/BeldexWalletSyncStatusListener")));
...
Java_io_oxen_coin_OxenApi_setupListenerJNI(JNIEnv *env, jobject inst, jobject listener) {
    listenerWrapper = new BeldexWalletListenerWrapper(env, listener);
